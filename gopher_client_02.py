...
class GopherClient:
    ...
    def crawl(self, selector='', depth=0):
        if depth > self.MAX_DEPTH:
            print(f"Max recursion depth reached at selector: {selector}")
            return
    ...
    def print_summary(self):
        ...
        for path, _ in self.binary_files:
            print(f"  {path}")
        print(f"Smallest text file content:\n{self.smallest_text[:500]}" if self.smallest_text else "No text files found.")
